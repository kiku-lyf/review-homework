...
深度学习的介入改变了“求解”这一概念。神经网络在本质上是高维函数逼近器，其参数化方式不依赖显式网格结构；只要能提供优化信号，就可以逼近 PDE 解或解算子。自从 Raissi 等提出 PINNs 后，“把 PDE 残差当作训练监督”的想法使得无标签求解成为可能，并迅速扩展到逆问题、参数识别与多物理场耦合\cite{查文舒2022基于神经网络的偏微分方程求解方法研究综述,李道伦2023基于神经算子与类物理信息神经网络智能求解新进展}。与此同时，算子学习路线在“大量仿真数据 $\to$ 学习整体解映射”的框架下得到突破，DeepONet 的结构化算子网络\cite{李慧2025deeponet,王云朋2025基于神经算子的湍流模拟方法}与 FNO 的频域卷积\cite{卫兰2025基于傅里叶神经算子的遥感数据预测方法,李谌2024嵌入傅里叶神经算子的卷积自编码声波速度反演方法}让模型具备离散无关（resolution-invariant）和跨参数泛化能力，显著改变了代理建模与实时推理的性能上限\cite{lu2022learning_operators_survey,kovachki2023neural}。

\section{研究意义}
从理论视角看，深度学习 PDE 求解把“数值分析中的逼近理论”与“机器学习的泛化理论”直接连接起来。PINNs、Deep Ritz、Deep Galerkin 等方法把残差最小化、能量泛函最小化与弱形式投影转化为可微优化问题，使神经网络成为新的函数空间基底\cite{karniadakis2021physicsinformed,liu2023pinn_stiff_cn}。神经算子则从函数空间到函数空间学习映射，扩展了传统降阶模型（ROM）在非线性与高维情况下的适用边界，并激发了对算子逼近、误差上界与样本复杂度的系统研究\cite{chen1995universal,lu2021deeponet,lanthaler2022error_fno}。
...
\section{传统数值方法回顾}
传统数值方法求解偏微分方程的基本思路是将连续问题离散化。有限差分法（FDM）基于 Taylor 展开理论，采用差商近似偏导数；有限元法（FEM）基于变分原理，在单元内采用基函数近似未知函数；有限体积法（FVM）则基于控制体积的积分守恒。传统方法的误差主要来源于空间离散的截断误差、时间离散的截断误差以及迭代求解的数值误差。相比之下，深度学习方法采用神经网络 $u_\theta$ 作为解的参数化表示，其误差来源转变为函数逼近误差与优化误差\cite{brunton2020datadriven_pde,rudy2017datadriven_pde}。这种范式转换使得求解问题的核心从“离散稳定性分析”转变为“优化景观设计与梯度尺度匹配”。

\section{深度学习方法的基本思想与分类}
深度学习求解偏微分方程的核心思想，是将原本依赖数值离散与迭代求解的 PDE 问题转化为神经网络参数优化问题。通过以神经网络 $u_{\theta}$ 或算子映射 $G_{\theta}$ 参数化未知函数或解算子，求解过程不再依赖固定网格或显式离散格式，而是在连续空间中通过端到端优化自动学习 PDE 的解结构或解映射规律。这一范式的根本变化，使得深度学习方法能够在高维空间、复杂几何以及参数化 PDE 等传统数值方法难以处理的场景中展现显著优势\cite{sun2021physics_dl_cn,li2021physicsinformed}。

根据物理信息利用方式与任务目标的不同，深度学习求解 PDE 的方法大体可分为以下三类：

(1)数据驱动方法（Data-Driven Methods）数据驱动方法完全依赖高保真模拟数据或实验数据，通过监督学习直接拟合输入与输出场之间的映射关系。例如，卷积神经网络（CNN）或 U-Net \cite{ronneberger2015u}可用于流场重建\cite{falkner2018u_net,thuerey2020deepcfd}、湍流超分辨率，Transformer\cite{vaswani2017attention} 则用于捕捉长程依赖的物理过程。这类方法的优势在于推理速度极快，适用于代理建模、实时仿真或快速参数扫描等任务。但由于缺乏显式物理约束，其泛化能力依赖于训练数据覆盖范围，一旦遇到分布外场景，可能产生无法解释或物理不一致的预测结果。
...
\section{PINN基本原理}
物理信息神经网络（PINNs）由 Raissi 等人于 2019 年提出\cite{raissi2019physics}，其核心思想是将偏微分方程作为约束条件融入神经网络的损失函数中，随后被广泛综述并工程化实现\cite{查文舒2022基于神经网络的偏微分方程求解方法研究综述,lu2021deepxde,liu2021pde_dl_cn}。考虑一般形式的偏微分方程：
\begin{equation}
...
针对 PINN 训练过程中的频谱偏置、梯度竞争与多尺度数值病态等系统性挑战，研究者在近年来逐步形成了一套可复用的改进策略体系。这些改进方法可从损失函数设计、采样策略优化、网络架构增强、求解域分解以及约束形式转换等多个维度进行系统分类\cite{wang2021understanding_pinn,krishnapriyan2021characterizing}.

\subsection{自适应权重与梯度均衡策略}

第一类改进思路聚焦于损失函数中多项约束的权重动态调整，以解决梯度竞争导致的优化失衡\cite{liu2023pinn_stiff_cn}问题。经典 PINN 采用固定的损失权重组合，这要求研究者预先对不同物理项的重要性进行人工标定，不仅繁琐且难以保证在整个训练过程中的适配性。为此，自适应权重方法应运而生，其核心思想是根据训练阶段动态调节各损失项的有效梯度贡献，使物理残差、初值条件、边界条件等多项约束在优化过程中保持同量级的梯度范数，从而显著提升收敛稳定性与最终精度\cite{mcclenny2020selfadaptive,wang2022and,lu2021causal_pinn}。
...
当 PDE 解不够光滑（如含弱间断）或输入数据包含噪声时，强形式 PINN 所依赖的逐点残差匹配需要精确计算高阶导数，这会显著放大自动微分过程中的数值噪声，导致训练极其脆弱甚至失败。弱形式与变分 PINN 的核心思想是将 PDE 转化为变分形式或弱形式，用加权积分（通过引入测试函数 $v$）替代逐点导数匹配，从而降低对解的光滑性要求，同时使训练信号对局部不光滑更具鲁棒性\cite{kharazmi2019hpvpinn,kharazmi2021vpinn,baltar2022weak_pinn,mao2020vpinn}。
...
\section{频域视角与 Fourier Neural Operator}
...
卷积定理（Convolution Theorem）揭示了空间域卷积与频域点乘之间的等价关系\cite{li2021fourier}：
\begin{equation}
...
FNO 的频域设计带来了三个层面的显著优势，使其在算子学习领域迅速成为主流方法之一\cite{卫兰2025基于傅里叶神经算子的遥感数据预测方法,cao2021choose_fno,lanthaler2022error_fno}。
...
为了克服这一局限，研究者提出了多种几何扩展方案。Geo-FNO 通过坐标映射（coordinate mapping）技术，将物理域 $\Omega_{\text{phys}}$ 变换至标准计算域 $\Omega_{\text{comp}}$（如单位正方形），在计算域上应用 FNO 后再映射回物理域，从而间接处理不规则几何\cite{li2022gino}。然而，这种方法对映射质量敏感，在存在极端变形或拓扑复杂性时可能失效。Deformation-FNO 进一步引入可学习的变形场，允许网络自适应调整频域表示以适应几何形状\cite{li2022gino}。GINO（Geometry-Informed Neural Operator） 则结合了图神经网络与傅里叶卷积，在结构化区域使用 FNO 高效捕捉全局模式，在不规则边界附近使用图卷积处理局部几何细节，实现了几何灵活性与计算效率的平衡\cite{li2022gino,li2020graphneuraloperator,chen2023gnn_pde_cn}。
...
\section{图/点云/几何神经算子：复杂域的可泛化代理}

\subsection{从规则网格到非结构化表示}

FNO 虽然在规则网格上实现了高效的全局算子学习，但其对快速傅里叶变换的依赖使其难以处理工程实际中广泛存在的复杂几何。真实物理系统往往定义在高度不规则的空间域上：机翼型面、车身外形、器官结构、多孔介质等，这些几何形状无法嵌入规则网格而不引入巨大计算浪费。传统 CFD 与 FEA 早已转向非结构化网格（Unstructured Grids），通过三角形、四面体等单元灵活拟合复杂边界。因此，构建能够天然处理非结构化数据的神经算子成为算子学习走向工程实用的关键。\cite{chen2023gnn_pde_cn,sun2023gnn_operator_cn}

图神经算子（Graph Neural Operator, GNO）的核心思想是将物理域的离散化表示从"规则网格阵列"转变为"图结构 $G=(V, E)$"，通过图神经网络的消息传递机制将连续核积分算子近似为图上的邻域聚合操作\cite{li2020graphneuraloperator,li2022gino,chen2023gnn_pde_cn,sun2023gnn_operator_cn}。
...
随着 Transformer 在序列建模中的成功，研究者开始探索注意力机制在算子学习中的物理可解释性。数学上，Self-Attention 机制中的 Attention Map 可以被严格解释为一种数据驱动的动态积分核 $\kappa(x,y)$\cite{cao2021choose_fno}。与 FNO 中固定的平移不变卷积核不同，注意力机制能够根据输入特征（Query 和 Key）动态调整积分权重：
\begin{equation}
...
\section{基于深度学习的数值方法}
这类方法将深度学习与传统数值方法结合，用神经网络改进数值方法的某些环节\cite{李想2011机器学习与计算力学的结合及应用初探}。例如，学习型有限差分方法用神经网络学习差分格式的系数，PDE-Net 使用可学习的卷积核近似微分算子，通过约束确保卷积核的微分性质。此外，Solver-in-the-Loop 方法将可微分的物理求解器与神经网络结合，通过端到端训练优化数值格式。这种混合范式保留了数值方法的稳定框架，仅将关键但难以手工设计的部件（如通量形式、预条件器）交给网络学习。
...
学习型有限元方法用神经网络优化基函数\cite{郭鹏2020基于深度强化学习与有限元仿真集成的拉深成形控制}，可以学习问题特定的基函数，提高计算效率。有限元法使用基函数近似未知函数，基函数的选择影响精度和效率。学习型有限元用神经网络优化基函数，可以学习问题特定的基函数，提高计算效率。Alet等人提出的图元网络（Graph Element Network）模仿有限元分析的行为，使用图神经网络建模空间关系，节点位置和连接都可以优化。
...
\section{生成式模型与扩散 PDE 求解}

\subsection{从确定性回归到概率分布建模}
传统的基于神经网络的 PDE 求解方法（如 PINNs 和 DeepONet）通常被构建为确定性的回归问题，即寻找一个单一的最优解函数以最小化残差或数据误差。然而，在面对反问题求解、高雷诺数湍流模拟以及数据稀缺场景下的不确定性量化（Uncertainty Quantification, UQ）时，点估计方法往往难以捕捉解的多模态分布特性。生成式模型（Generative Models）的引入标志着求解范式从“寻找单一解”向“模拟解的条件概率分布”转变\cite{zhao2022generative_pde_cn,liu2022pinn_uncertainty,yang2021bayesian_pinn}。通过学习数据分布 $p(u|y)$（其中 $u$ 为物理场，$y$ 为观测条件），生成式模型不仅能够重构高保真的物理场，还能通过采样直接评估预测的不确定性，为复杂物理系统的建模提供了新的统计力学视角\cite{zhao2022generative_pde_cn,huang2024diffusionpde}。

\subsection{早期探索：对抗生成与变分推断}
早期的研究主要集中在物理信息生成对抗网络（Physics-Informed GANs, PI-GANs）\cite{yang2020pigans}和变分自编码器（VAEs）。PI-GANs 通过在判别器中引入 PDE 残差项，迫使生成器产生的样本不仅符合数据分布，同时满足物理约束。这种对抗博弈机制在模拟随机渗透率场中的达西流（Darcy Flow）等问题上取得了一定成功。然而，GANs 固有的训练不稳定性（如模式坍塌）以及 VAEs 在生成高频纹理时的模糊性，限制了它们在精细流场模拟中的应用。尽管如此，这些工作证实了深度生成模型具备在无标签数据情况下，仅依靠物理控制方程生成符合物理规律解空间的能力\cite{huang2024diffusionpde}.

\subsection{扩散模型：随机微分方程的逆向求解}
近年来，去噪扩散概率模型（Denoising Diffusion Probabilistic Models, DDPM）\cite{ho2020denoising}和基于分数的生成模型（Score-based Generative Models, SGM）的兴起，为 PDE 求解带来了突破性进展\cite{huang2024diffusionpde,xu2020pinn_fluid_cn}。从理论层面看，扩散模型的正向过程本身就是一个由随机微分方程（SDE）描述的物理扩散过程（即热方程演化），其将结构化数据逐渐转化为高斯噪声；而逆向生成过程则对应于求解一个逆时间的 SDE。根据 Anderson 定理，这一逆向过程完全由数据分布的“分数函数”（Score Function，即对数概率密度梯度 $\nabla_x \log p(x)$）决定。因此，训练扩散模型本质上是在学习高维物理场分布的梯度场。这种严格的数学等价性使得扩散模型在处理高维、非线性 PDE 时表现出极高的稳定性和样本质量，特别是在生成具有精细相干结构的湍流场方面，其表现显著优于 GANs。
...
电磁学问题涉及Maxwell方程，是天线设计、电磁兼容等的基础\cite{查文舒2022基于神经网络的偏微分方程求解方法研究综述}。Maxwell方程描述了电磁场的行为，深度学习方法可以求解Maxwell方程，处理复杂几何和时变问题\cite{raissi2019physics,查文舒2022基于神经网络的偏微分方程求解方法研究综述}。PINN可以求解时域和频域Maxwell方程，预测电磁场分布，为天线设计、电磁兼容等问题提供快速求解工具\cite{raissi2019physics,查文舒2022基于神经网络的偏微分方程求解方法研究综述}。神经算子方法可以学习Maxwell方程的求解算子，实现快速推理，特别适合优化设计等需要大量重复求解的场景\cite{li2020neural_operator,li2021fourier,lu2021deeponet}。
...
\section{当前挑战}
尽管深度学习在求解偏微分方程（PDEs）方面取得了显著进展，但要真正替代或辅助传统数值方法进入工业级应用，仍面临着从理论基础到工程实践的多重挑战。现阶段的核心矛盾可以概括为“可训练性（Trainability）、泛化性（Generalizability）与可信度（Trustworthiness）”之间的权衡。

\subsection{理论分析的滞后性与优化困境}
目前关于 PINNs 和神经算子的理论研究多集中于通用逼近定理（Universal Approximation Theorem），证明了神经网络在理论上能够以任意精度逼近 PDE 的解或算子\cite{chen1995universal,kovachki2023neural}。然而，逼近能力并不等同于优化能力。PINN 的损失函数通常是非凸的，且包含多个相互竞争的物理约束项，这导致优化景观中存在大量局部极小值和鞍点，使得基于梯度的优化算法容易陷入次优解\cite{wang2021understanding_pinn}。此外，与传统有限元法（FEM）拥有完善的误差估计和收敛阶理论不同，深度学习方法尚缺乏针对特定 PDE 类型的先验误差界，目前难以准确预估在给定网络规模和样本量下的求解精度，这限制了其在航空航天等高安全要求领域的应用。

\subsection{高维与多尺度系统的计算瓶颈}
虽然深度学习被认为有望打破“维数灾难”，但在处理极端复杂系统时仍面临计算瓶颈。在湍流、多孔介质流动等问题中，物理过程跨越多个时空尺度（从 Kolmogorov 尺度到积分尺度），为了解析微小尺度的特征，神经网络需要极大的容量（参数量）和海量的训练配点，导致训练成本甚至超过直接数值模拟（DNS）。同时，对于具有快慢动力学耦合的刚性方程（如化学反应流），时间步长的选择受到极大约束，现有的时间步进 PINN 或连续时间模型往往难以在长时间积分中保持数值稳定性。

\subsection{数据依赖与分布外泛化}
神经算子（如 FNO, DeepONet）虽然实现了算子级的映射，但其性能高度依赖于训练数据的质量和覆盖范围。当测试案例的几何形状、边界条件或参数分布超出训练集覆盖范围时，模型的分布外（OOD）泛化能力往往会急剧下降，预测精度无法保证。此外，对于许多实际工程问题，获取大规模的高保真仿真数据以训练算子模型本身就是极其昂贵的，这种高昂的数据获取成本在一定程度上抵消了代理模型在推理阶段带来的速度优势。

\section{未来展望}
面对上述挑战，未来的研究将不再局限于单一网络架构的修补，而是向着“物理知识深度融合”、“基础模型化”和“混合计算范式”演进。

\subsection{物理基础模型（Physics Foundation Models）}
受自然语言处理（NLP）领域大语言模型（LLM）的启发，构建面向科学计算的“基础模型”将是未来的重要方向。通过在海量的多物理场数据（涵盖流体、固体、电磁等）上进行预训练，模型可以学习到通用的物理守恒律和时空演化模式。在下游任务中，只需通过少量的样本微调（Few-shot Finetuning）或提示工程（Prompt Engineering），即可适应特定的 PDE 求解任务。这种“预训练-微调”范式有望显著降低新问题的建模成本。

\subsection{神经-数值混合求解器（Hybrid Neuro-Numerical Solvers）}
深度学习与传统数值方法并非对立关系，而是互补关系，未来的主流求解器将是两者的深度融合。一方面，可以利用神经网络辅助传统求解，加速其瓶颈环节，例如学习最优的网格划分策略、构建高效的预条件子（Preconditioners）或替代复杂的物理闭包模型（如湍流模式）。另一方面，可微物理编程将成为重要趋势，即将传统数值离散格式编写为可微分算子并嵌入到深度学习框架中。这种“物理可微层”的设计既保留了数值方法的理论保证，又具备深度学习的优化灵活性，实现了数据驱动与物理机理的双向赋能。

\subsection{可解释性与物理守恒的内嵌}
为了提高模型的可信度，必须从“软约束”走向“硬约束”。除了在损失函数中添加惩罚项外，未来的网络架构将更多地利用群论、微分几何等数学工具，构建具有 $SE(3)$ 等变性、辛结构保全或严格质量守恒的神经网络架构（如 HNN, SympNets）。同时，开发针对科学机器学习的可解释性工具，分析网络内部关注的物理特征，对于发现新物理规律具有重要意义。

% ----------------------------------------------------------------------
% 第八章 结论
% ----------------------------------------------------------------------
\chapter{结论}
本文系统综述了深度学习方法在求解偏微分方程领域的研究进展，涵盖了从基础理论、核心算法到工程应用的全貌。在方法论革新方面，物理信息神经网络（PINNs）通过将物理方程作为正则化约束，实现了无网格、无标签的求解范式，特别适用于逆问题和高维问题；而神经算子（Neural Operators）通过学习无限维函数空间之间的映射，实现了分辨率无关的快速推理，为实时仿真和参数化设计提供了新途径。在应用广度上，这些方法已成功渗透到流体力学、固体力学、多物理场耦合等多个学科，解决了一系列传统方法难以处理的复杂问题，如稀疏数据下的流场重建、非结构化网格上的快速预测等。尽管深度学习方法在精度和理论完备性上仍有待完善，但它并非要完全取代传统数值方法，而是作为一种强大的补充工具，与之形成“双引擎”驱动。

展望未来，随着物理基础模型的兴起、混合计算架构的成熟以及数学理论的完善，深度学习求解 PDE 将从一种“实验性尝试”转变为科学计算领域的“标准范式”之一，在数字孪生、自动驾驶、气候预测及先进制造等关键领域发挥不可替代的作用。科学计算的未来，必将是物理机理与数据智能深度融合的时代。
